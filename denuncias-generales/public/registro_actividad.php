<?php
/**
 * Portal Ciudadano de Denuncias - Registro de Actividad
 */
...
function getActionStyle($action) {
    $map = [
        ...
        'denuncia_creada' => ['icon' => 'bi-plus-circle', 'color' => '#1a6591', 'bg' => 'rgba(26,101,145,0.1)', 'label' => 'Denuncia creada'],
        ...
        'usuario_creado' => ['icon' => 'bi-person-plus', 'color' => '#2380b0', 'bg' => 'rgba(35,128,176,0.1)', 'label' => 'Usuario creado'],
...
.stat-card-activity:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(0,0,0,0.08); }
.stat-card-activity .stat-icon { width: 48px; height: 48px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.3rem; }
.stat-card-activity .stat-number { font-size: 1.8rem; font-weight: 700; line-height: 1; }
.activity-item {
    display: flex; gap: 16px; padding: 16px 20px;
    border-bottom: 1px solid rgba(255,255,255,0.04); transition: all 0.2s; cursor: default;
}
...
.activity-item:hover .activity-icon-wrap { transform: scale(1.1); }
.activity-action-label { font-weight: 600; font-size: 0.9rem; color: #1e293b; }
.activity-meta { font-size: 0.8rem; color: rgba(255,255,255,0.5); margin-top: 2px; }
.activity-detail-text { font-size: 0.82rem; color: rgba(255,255,255,0.55); margin-top: 4px; background: rgba(255,255,255,0.04); padding: 6px 10px; border-radius: 8px; border-left: 3px solid rgba(255,255,255,0.12); }
.activity-time { font-size: 0.78rem; color: rgba(255,255,255,0.5); white-space: nowrap; text-align: right; min-width: 100px; }
.filter-panel { background: rgba(255,255,255,0.06); border-radius: 16px; border: 1px solid rgba(255,255,255,0.1); padding: 20px; margin-bottom: 24px; }
.filter-panel .form-control, .filter-panel .form-select { border-radius: 10px; font-size: 0.88rem; border-color: #e2e8f0; }
.filter-panel .form-control:focus, .filter-panel .form-select:focus { border-color: #1a6591; box-shadow: 0 0 0 3px rgba(26,101,145,0.1); }
.badge-action { font-size: 0.72rem; font-weight: 600; padding: 4px 10px; border-radius: 6px; display: inline-flex; align-items: center; gap: 4px; }
.pagination-modern .page-link { border: none; border-radius: 10px; margin: 0 2px; color: rgba(255,255,255,0.55); font-weight: 500; font-size: 0.88rem; padding: 8px 14px; }
.pagination-modern .page-item.active .page-link { background: #1a6591; color: white; }
.pagination-modern .page-link:hover { background: rgba(255,255,255,0.04); color: #1a6591; }
...
<div class="main-content">
    <div class="d-flex flex-wrap justify-content-between align-items-start mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Auditoría completa del portal &middot; <?= number_format($totalRows) ?> evento(s)</p>
        </div>
        ...
        <div class="col-6 col-lg-3">
            <div class="stat-card-activity">
                <div class="d-flex align-items-center gap-3">
                    <div class="stat-icon" style="background: rgba(26,101,145,0.1); color: #1a6591;">
                        <i class="bi bi-calendar-day"></i>
                    </div>
                    <div><div class="stat-number text-dark"><?= $todayCount ?></div><div class="text-muted small">Hoy</div></div>
                </div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
            <div class="stat-card-activity">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div><div class="stat-number text-dark"><?= $weekCount ?></div><div class="text-muted small">Última semana</div></div>
                </div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
            <div class="stat-card-activity">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div><div class="stat-number text-dark"><?= $loginCount ?></div><div class="text-muted small">Logins (24h)</div></div>
                </div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
            <div class="stat-card-activity">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div><div class="stat-number text-dark"><?= $uniqueUsers ?></div><div class="text-muted small">Usuarios activos (24h)</div></div>
                </div>
            </div>
        </div>
    </div>
        ...
        <form method="GET" class="row g-3 align-items-end">
            <div class="col-md-3">
                ...
                <div class="input-group">
                    <span class="input-group-text bg-dark-blue border-end-0" style="border-radius:10px 0 0 10px;"><i class="bi bi-search text-muted"></i></span>
    ...
    <div class="card border-0 shadow-sm" style="border-radius: 16px; overflow: hidden;">
            ...
            <div class="activity-item" style="animation-delay: <?= $i * 0.04 ?>s">
                ...
                <div class="activity-details" style="flex:1; min-width:0;">
                    ...
                    <div class="activity-meta">
                        ...
                        <?php endif; ?>
                        &middot; <i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($log['ip_address'] ?? '-') ?>
                    </div>
                    <?php if (!empty($log['details'])): ?>
                    <div class="activity-detail-text"><?= htmlspecialchars($log['details']) ?></div>
                    <?php endif; ?>
                </div>
        ...
        <?php if ($totalPages > 1): ?>
        <div class="p-3 bg-dark-blue border-top">
            <nav>
