<?php
/**
 * Portal Ciudadano de Denuncias - Notificaciones
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($ajaxAction === 'mark_read') {
        ...
        $pdo->prepare("UPDATE notifications SET is_read = 1, read_at = NOW() WHERE id = ? AND user_id = ?")->execute([$nid, $userId]);
    ...
    if ($ajaxAction === 'update_email' && $isAdmin) {
        ...
        if (!$targetUserId) { echo json_encode(['ok' => false, 'error' => 'Usuario inválido']); exit; }
        if (!$newEmail) { echo json_encode(['ok' => false, 'error' => 'Email inválido']); exit; }
        $check = $pdo->prepare("SELECT id FROM users WHERE email = ? AND id != ?");
        $check->execute([$newEmail, $targetUserId]);
        if ($check->fetch()) { echo json_encode(['ok' => false, 'error' => 'Este email ya está en uso por otro usuario']); exit; }
    ...
    if ($ajaxAction === 'test_email' && $isAdmin) {
        ...
        if (!$targetUser || empty($targetUser['email'])) {
            echo json_encode(['ok' => false, 'error' => 'Usuario sin email configurado']); exit;
        }
        try {
            $testContent = '
    ...
    if ($ajaxAction === 'save_subscriptions' && $isAdmin) {
        ...
        if (!$targetUserId || !is_array($groupIds)) { echo json_encode(['ok' => false, 'error' => 'Datos inválidos']); exit; }
        $userExists = $pdo->prepare("SELECT id FROM users WHERE id = ?");
        $userExists->execute([$targetUserId]);
        if (!$userExists->fetch()) { echo json_encode(['ok' => false, 'error' => 'Usuario no encontrado']); exit; }
        ...
        try {
            ...
            foreach ($groupIds as $gid) { $ins->execute([$targetUserId, (int)$gid]); }
            $pdo->commit();
...
if ($isAdmin) {
    ...
    foreach ($allSubs as $sub) { $userSubscriptions[$sub['user_id']][] = $sub['group_id']; }
}

function getGroupStyle($slug) {
    $map = [
        'todas' => ['icon' => 'bi-bell-fill', 'color' => '#8b5cf6', 'bg' => 'rgba(139,92,246,0.1)'],
        'denuncia_creada' => ['icon' => 'bi-plus-circle-fill', 'color' => '#1a6591', 'bg' => 'rgba(26,101,145,0.1)'],
...
function getRoleBadge($role) {
    $map = [
        ...
        'investigador' => '<span class="badge" style="background:#f59e0b; color:#000; font-size:0.7rem;">Delegado</span>',
...
.save-btn-wrap { opacity: 0; transform: translateY(10px); transition: all 0.3s; pointer-events: none; }
.save-btn-wrap.visible { opacity: 1; transform: translateY(0); pointer-events: all; }
</style>
...
<div class="main-content">
    ...
    <div class="card border-0 shadow-sm" style="border-radius: 16px; overflow: hidden;">
        ...
        <?php if ($totalPages > 1): ?>
        <div class="p-3 bg-dark-blue border-top text-center">
            <nav>
    ...
    <!-- TAB: Gestionar Suscripciones (admin) -->
...
function saveSubscriptions(userId) {
    ...
    fetch('/notificaciones', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: `ajax_action=save_subscriptions&target_user_id=${userId}&group_ids=${encodeURIComponent(JSON.stringify(groupIds))}&<?= CSRF_TOKEN_NAME ?>=${csrfToken}`
    }).then(r => r.json()).then(d => {
        if (d.ok) {
            ...
            btn.disabled = false;
            setTimeout(() => { btn.innerHTML = originalText; btn.classList.replace('btn-danger', 'btn-primary'); }, 2000);
        }
    });
}
...
function saveEmail(userId, btn) {
    ...
    fetch('/notificaciones', {
        ...
        feedback.style.fontSize = '0.75rem';
        if (d.ok) {
            feedback.className = 'email-feedback mt-1 text-info';
