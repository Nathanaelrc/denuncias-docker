...
.epco-nav .nav-link-u{color:rgba(255,255,255,.7);text-decoration:none;padding:6px 14px;border-radius:8px;font-size:.85rem;transition:all .2s;display:flex;align-items:center;gap:5px}
.epco-nav .nav-link-u:hover{color:#fff;background:rgba(255,255,255,.08)}
.epco-nav .nav-link-u.active{color:#fff;background:rgba(255,255,255,.12);font-weight:600}
...
@media(max-width:991px){
    ...
    .epco-nav .nav-link-u,.epco-nav .portal-pill{width:100%;justify-content:center}
...
<nav class="epco-nav">
    ...
    <div class="epco-nav-collapse" id="navCollapseK">
        <a class="nav-link-u" href="<?= htmlspecialchars($_landingUrl) ?>">
            <i class="bi bi-arrow-left"></i>Inicio
        </a>
        <span class="sep-dot">|</span>
        <span class="portal-pill active"><i class="bi bi-shield-check"></i>Ley Karin</span>
        <a class="portal-pill" href="<?= htmlspecialchars($_generalUrl) ?>"><i class="bi bi-globe2"></i>Ciudadano</a>
        <span class="sep-dot">|</span>
        <a class="nav-link-u <?= $_navUri === '/nueva_denuncia' ? 'active' : '' ?>" href="/nueva_denuncia">
            <i class="bi bi-pencil-square"></i>Realizar Denuncia
        </a>
        <a class="nav-link-u <?= $_navUri === '/seguimiento' ? 'active' : '' ?>" href="/seguimiento">
            <i class="bi bi-search"></i>Seguimiento
        </a>
    </div>
    <div class="epco-nav-right">
        <?php if (isLoggedIn()): ?>
            <a href="/panel" class="btn-acceso"><i class="bi bi-speedometer2"></i>Dashboard</a>
            <a href="/cerrar_sesion" class="btn-acceso"><i class="bi bi-box-arrow-right"></i>Salir</a>
        <?php else: ?>
            <a href="/acceso" class="btn-acceso"><i class="bi bi-box-arrow-in-right"></i>Iniciar Sesión</a>
        <?php endif; ?>
    </div>
</nav>
